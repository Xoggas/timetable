...
export class LessonListService {
	...
	async update(updateLessonDto: UpdateLessonDto): Promise<Lesson> {
		return await this.lessonModel.findByIdAndUpdate({ _id: updateLessonDto.id }, { name: updateLessonDto.name }).exec();
	}

	async delete(deleteLessonDto: DeleteLessonDto): Promise<Lesson> {
		return await this.lessonModel.findByIdAndDelete({ _id: deleteLessonDto.id });
	}
}
